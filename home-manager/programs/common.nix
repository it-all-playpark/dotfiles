{
  shellSortcuts = {
    ...
    # merge済みworktree一括削除（MERGED / PR無し+default branchにマージ済み のみ削除、
    # gh障害・dirty・OPEN・未マージは保護、pid死亡のstaleロックは自動unlock）
    # 実体: home-manager/home/file/bin/git-worktree-gc → ~/.local/bin/git-worktree-gc
    wrm = "~/.local/bin/git-worktree-gc";
    # 全repo横断（ghq list 配下）
    wrma = "~/.local/bin/git-worktree-gc --all";
    # 全repo横断の判定結果だけ表示（削除しない）
    wrmn = "~/.local/bin/git-worktree-gc --all --dry-run";
