...
#
# 目的:
#   hook から「read_only か / どの失敗種別か」のような、正規表現では書けないが
#   答えが有界（選択肢・yes/no・段階）な意味判定を安価に得る。Jev は文章を
#   生成せず、各選択肢の較正済み確率を返す。判定結果はログのラベル付けに
#   使い、permission の allow/deny 自体は既存の決定論 hook に任せる。
#
# 経路:
